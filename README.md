...
换 chunk 时，默认使用 `--chunk-blend-steps 6`：后续 chunk 的前 6 步将关节目标从已下发的 hold
位置平滑混合到对应模型目标，第 6 步完全恢复模型轨迹。短 chunk 使用其实际长度作为过渡步数。
30 Hz 下过渡约 200 ms，使用原有动作步，不增加等待或动作数量；首段和夹爪不参与混合。
这会调整新段开头的关节轨迹，但不消除等待推理的停顿。设为 `0` 可恢复原先直接衔接。
控制日志的 `blended_action` 记录混合后的目标，`queued_action` 保留模型原始目标。
