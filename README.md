...
| 可选 | `--chunk-smoothing-strength` | `0.5` | 对每个动作块内部的双臂关节做一次三点平滑；保留首尾动作和夹爪原值。取值范围 `[0,1]`；`0` 关闭。 |
| 可选 | `--chunk-stop-steps` | `6` | 在每个动作块最后 N 步对双臂关节做减速，并在正常结束前 hold N 个控制周期；不处理夹爪。30 Hz、6 步约为 `200 ms`；`0` 关闭。 |
