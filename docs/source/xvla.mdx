...
**During Training**: `policy.domain_id` defaults to 0. New embodiments should reserve an unused ID and keep it
identical in training and deployment. The processor stores this value in the checkpoint pipeline.
...
domain_id: int = 0              # Soft-prompt/domain table entry used by the processor
